...
RUN git config --global url."ssh://git@gitlab.uufff.com/".insteadOf "https://gitlab.uufff.com/" && \
    mkdir -p ~/.ssh && \
    cp ./devops/ssh/<your-private-key>* ~/.ssh/ && \
    chmod 600 ~/.ssh/<your-private-key>* && \
    mv ~/.ssh/<your-private-key> ~/.ssh/id_rsa && \
    mv ~/.ssh/<your-private-key>.pub ~/.ssh/id_rsa.pub && \
    touch ~/.netrc && \
    echo "machine gitlab.uufff.com login <your-username> password <your-token-or-password>" > ~/.netrc && \
    touch ~/.ssh/known_hosts && \
    chmod 600 ~/.ssh/known_hosts && \
    ssh-keyscan gitlab.uufff.com >> ~/.ssh/known_hosts
```

> 不要把真实用户名、Token、密码或私钥文件名提交到仓库文档中。
