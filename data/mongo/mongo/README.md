...
`mongo` 目录提供包 `mongopkg`，用于在 Go 服务中创建 MongoDB v2 驱动客户端、挂载 Mongo command monitor、记录慢查询日志，并复用常见 Mongo 操作常量。
...
```go
import mongopkg "github.com/ikaiguang/go-srv-kit/data/mongo/v3/mongo"
```
...
import (
	...
	mongopkg "github.com/ikaiguang/go-srv-kit/data/mongo/v3/mongo"
...
func openMongo(ctx context.Context) error {
	...
	client, err := mongopkg.NewMongoClient(cfg, slog.Default())
	if err != nil {
		return err
	}
...
不要手工修改生成文件。修改 `config.proto` 后按仓库 Makefile 目标重新生成：
