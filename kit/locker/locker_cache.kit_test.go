...
func TestCache_LockOnce(t *testing.T) {
	...
	t.Run("首次加锁成功", func(t *testing.T) {
		...
		require.NotNil(t, unlock, "unlock 不应为 nil")

		// 解锁
		ok, err := unlock.Unlock(ctx)
		assert.True(t, ok, "解锁应成功")
		assert.Nil(t, err, "解锁不应报错")
	})

	// 等待锁过期后再次加锁应成功
	t.Run("锁过期后再次加锁成功", func(t *testing.T) {
		unlock1, err := locker.Once(ctx, cacheKeyName+"_expire")
		require.Nil(t, err, "加锁应成功")
		...
		// 过期后再次加锁应成功
		unlock2, err := locker.Once(ctx, cacheKeyName+"_expire")
...
func TestCache_LockMutex(t *testing.T) {
	...
	t.Run("首次加锁成功", func(t *testing.T) {
		unlock, err := locker.Mutex(ctx, cacheKeyName+"_mutex")
		require.Nil(t, err, "首次 Mutex 加锁应成功")
		require.NotNil(t, unlock)

		// 持有锁期间再次加锁应失败
		t.Run("持有锁期间再次加锁失败", func(t *testing.T) {
			_, err := locker.Mutex(ctx, cacheKeyName+"_mutex")
	...
	t.Run("解锁后再次加锁成功", func(t *testing.T) {
		unlock1, err := locker.Mutex(ctx, cacheKeyName+"_mutex2")
		require.Nil(t, err)
		require.NotNil(t, unlock1)
		...
		// 解锁后应能再次加锁
		unlock2, err := locker.Mutex(ctx, cacheKeyName+"_mutex2")
